...
if(isset($_FILES['image']) && $_FILES['image']['tmp_name']!='')
  {	
	...
	$sub = $_POST['subject'];
	$desc = $_POST['information'];
	$image = $admin->get_article_img_by_id($id);
	$image_url = dirname(dirname(__FILE__)) . '/users_img/'.$image['at_img'];
	@unlink($image_url);
...
/*if(file_exists($targetFile)){
  ...
  move_uploaded_file($tempFile,$kk);
	  ...
	  $image = $admin->get_article_img_by_id($id);
	  $newname = $image['at_img'];
	  
  }
	  
  $admin->update_article($id, $sub, $newname, $desc);
  
  
	header("Location: edit_articles.php?id=$id");
