...
class Users{
	...
	public function user_info_register_platinum($user_id, $address, $info, $category, $subcat, $url, $tube, $prod_desc, $timing, $staff, $lat, $lon) {
		
		$query 	= $this->db->prepare("INSERT INTO `company_info` (`cat_id`, `info_address`, `info_information`, `user_id`, `subc_id`, `info_url`, `info_tube`, `info_prod_desc`, `info_timing`, `info_staff`, `info_lat`, `info_lon`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ");
		...
		$query->bindValue(11, $lat);
		$query->bindValue(12, $lon);
