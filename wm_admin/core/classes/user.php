...
class Users{
	...
	public function register($username, $password, $email, $cat, $comp_name, $number_1,$number_2, $city, $pass,$person_name,$person_type){
		global $bcrypt;
		...
		$query 	= $this->db->prepare("INSERT INTO `users` (`user_name`, `user_pass`, `user_email`,  `user_comp_name`, `user_number`, `user_number2`,`user_time`, `user_ip`, `client_cat_id`, `city`,`pass`,`person_name`,`person_type` ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?) ");
		...
		$query->bindValue(5, $number_1);

		$query->bindValue(6, $number_2);
		$query->bindValue(7, $time);
		$query->bindValue(8, $ip);
		$query->bindValue(9, $cat);
		$query->bindValue(10, $city);
		$query->bindValue(11, $pass);
		$query->bindValue(12, $person_name);
		$query->bindValue(13, $person_type);
	...
	public function register_free($username, $password,$email, $cat, $comp_name, $number,$number2 ,$city, $pass, $user_aprov = 1, $admin_aprov = 1, $person_name,$person_type){
		global $bcrypt;
		...
		$query 	= $this->db->prepare("INSERT INTO `users` (`user_name`, `user_pass`, `user_email`,  `user_comp_name`, `user_number`,`user_number2`, `user_time`, `user_ip`, `client_cat_id`, `city`, `pass`, `admin_confirmed`, `user_confirmed`, `person_name`, `person_type`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ");
		...
		$query->bindValue(6, $number2);
		$query->bindValue(7, $time);
		$query->bindValue(8, $ip);
		$query->bindValue(9, $cat);
		$query->bindValue(10, $city);
		$query->bindValue(11, $pass);
		$query->bindValue(12, $user_aprov);
		$query->bindValue(13, $admin_aprov);
		$query->bindValue(14, $person_name);
		$query->bindValue(15, $person_type);
	...
	public function get_usersbyclient() {

		$query = $this->db->prepare("SELECT * FROM `users` WHERE `client_cat_id` <>1");
	...
	public function activate_blog($status, $id) {
		
				
				$query_2 = $this->db->prepare("UPDATE `blogs` SET `admin_confirmed` = ? WHERE `blogs_id` = ?");
	...
	public function deactivate_blog($status, $id) {
	...
	public function get_sub_admin($id){
	$query = $this ->db->prepare("SELECT * FROM `sub_admin` WHERE `id`= ?");
		$query->bindValue(1, $id);
	try{
	...
	public function user_info_register($id, $address, $info,$cat, $subcat,$landmark) {
		
		$query 	= $this->db->prepare("INSERT INTO `company_info` (`cat_id`, `info_address`, `info_information`,`user_id`, `subc_id`,`info_landmark`) VALUES (?, ?, ?, ?, ?, ? ) ");
	...
	public function user_info_register_platinum($user_id, $address, $info,$category, $subcat, $url, $tube, $prod_desc,$add_info,$landmark,$location) {
		
		//echo "Hello";
		$query 	= $this->db->prepare("INSERT INTO `company_info` (`cat_id`, `info_address`, `info_information`,`user_id`, `subc_id`, `info_url`, `info_tube`, `info_prod_desc`,`info_aditional`,`info_landmark`,`info_location`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ");
		...
		$query->bindvalue(9, $add_info);
		$query->bindvalue(10, $landmark);
		$query->bindvalue(11, $location);
...
FROM  `subcategories` 
LEFT JOIN categories ON categories.cat_id = subcategories.cat_id WHERE `subc_id`= ?");
		$query->bindValue(1, $id);
		try{
			$query->execute();
